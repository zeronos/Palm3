...
if (isset($_POST['add'])) {
    ...
    $farmer = $_POST['farmer'];
    $namefarm = preg_replace('/[[:space:]]+/', ' ', trim($_POST['fname']));
    $aliasfarm = preg_replace('/[[:space:]]+/', ' ', trim($_POST['falias']));
    //[province] => 65 [distrinct] => 804 [subdistrinct] => 6472 
    //[request] => insert [st] => [prov] => กระบี่ [dist] => คลองท่อม [subdist] => คลองท่อมเหนือ [id_prov] => 65 [id_dist] => 804 [id_subdist] => 6472
    $addfarm = preg_replace('/[[:space:]]+/', ' ', trim($_POST['address']));
    $subdistrinct = $_POST['subdistrinct'];
    $dataAddress = selectData("SELECT * FROM `db-subdistrinct` WHERE AD3ID=".$subdistrinct);
    //print_r($dataAddress);
    $sql = "INSERT INTO `db-farm` ( 
        `Name`, `Alias`, `Icon`, `Address`, 
        `AD3ID`, `UFID`, `IsCoordinate`, `Latitude`, `Longitude`
        ) VALUES ( 
        '".$namefarm."', '".$aliasfarm."', 'default', '".$addfarm."', 
        '".$subdistrinct."', '".$farmer."', '0', '".$dataAddress[1]['Latitude']."', '".$dataAddress[1]['Longitude']."')";
    //echo $sql;
    $IDFarm = addinsertData($sql);
    $LOG_LOGIN = $_SESSION[md5('LOG_LOGIN')];
    $Date = getDIMDate();
    $DIMFarmer = getDIMFarmer($farmer);
    $sql = "INSERT INTO `dim-farm` (
        `ID`, `IsFarm`, `dbID`, `Name`, `Alias`
        ) VALUES (
        NULL, '1', '$IDFarm', '$namefarm', '$aliasfarm');";
    //echo $sql;
    $IDDIMFarm = addinsertData($sql);
    $DIMAddr = getDIMAddr($subdistrinct, $addfarm);
    $time = time();
    $sql = "INSERT INTO `log-farm` (
        `ID`, `LOGloginID`, `StartT`, `StartID`, `EndT`, 
        `EndID`, `DIMownerID`, `DIMfarmID`, `DIMSubfID`, 
        `DIMaddrID`, `IsCoordinate`, `Latitude`, `Longitude`, 
        `NumSubFarm`, `NumTree`, `AreaRai`, `AreaNgan`, `AreaWa`, `AreaTotal`
        ) VALUES (
        NULL, '".$LOG_LOGIN[1]['ID']."', '".$time."', '".$Date[1]['ID']."', NULL, 
        NULL, '".$DIMFarmer[1]['ID']."', '".$IDDIMFarm."', NULL, 
        '".$DIMAddr[1]['ID']."', '0', '".$dataAddress[1]['Latitude']."', '".$dataAddress[1]['Longitude']."', 
        '0', '0', '0', '0', '0', '0')";
    //echo $sql;
    addinsertData($sql);
...
if (isset($_POST['delete'])) {
    ...
    $time = time();
    $DIMDATE = getDIMDate();
    $sql = "UPDATE `log-farm` SET 
        `EndT` = '".$time."', 
        `EndID` = '".$DIMDATE[1]['ID']."'  
        WHERE `log-farm`.`ID` IN ( SELECT * FROM( 
            SELECT `log-farm`.`ID` 
            FROM `log-farm` 
            INNER JOIN `dim-farm` ON `dim-farm`.`ID` = `log-farm`.`DIMfarmID` 
            WHERE `dim-farm`.`dbID`='".$FID."' AND `log-farm`.`EndT` IS NULL) tblTmp)  ";
    updateData($sql);
    echo $sql . "<br/>";
    $sql = "DELETE FROM `db-coorfarm` 
        WHERE `db-coorfarm`.`FCID`IN ( SELECT * FROM( 
            SELECT `db-coorfarm`.`FCID` 
            FROM `db-coorfarm` 
            INNER JOIN `db-subfarm` ON  `db-coorfarm`.`FSID` = `db-subfarm`.`FSID` 
            WHERE `db-subfarm`.`FMID` = '".$FID."') tblTmp)";
    delete($sql);
    echo $sql . "<br/>";
