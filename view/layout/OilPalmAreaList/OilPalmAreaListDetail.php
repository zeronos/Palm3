...
if (isset($_GET[('id')]) && isset($_GET[('fmid')]) && isset($_GET[('fname')]) && isset($_GET[('logid')])) {
...
WHERE `log-farm`.`DIMSubfID` IS NOT null AND `dim-farm`.`Name` = '" . $id . "'";
        ...
        <div class="col-xl-6 col-12">
            <div class="card">
                <div class="card-body" id="for_card">
                    <div class="row">
                        <img class="img-radius img-profile" src="../../<?php echo $idfarm[1]['Path']; ?>/<?php echo $idfarm[1]['FileName']; ?>" />
                    </div>
                    <div class="row mt-3">
        ...
        <div class="col-xl-6 col-12">
            <div class="card">
                <div class="card-body" id="card_height">
                    <div class="row">
                        <img class="img-radius img-profile" src="../../<?php echo $idfarmer[1]['Path']; ?>/<?php echo $idfarmer[1]['FileName']; ?>" />
                    </div>
                    <div class="row mt-3 justify-content-center">
                        <div class="col-xl-3 col-3 text-right">
                            <span>เกษตรกร : </span>
                ...
                if (this.readyState == 4 && this.status == 200) {
                    ...
                    for (i = 1; i <= data[0].numrow; i++) {
                        text += `<tr>
                            ...
                            <a href='OilPalmAreaListSubDetail.php?fmid=<?= $fmid ?>&SFID=${data[i].dbID}&nfarm=${data[i].nFarm}&nsubfarm=${data[i].Name}&farmer=${data[i].Alias}&logid=${data[i].ID}&numtree=${data[i].NumTree}'><button type='button' id='btn_info' class='btn btn-info btn-sm'><i class='fas fa-bars'></i></button></a>
    ...
    function initMap() {
        ...
        var triangleCoords = [
            ...
            for ($i = 1; $i < count($manycoor); $i++) {
                echo "
            {
                lat: '" . $manycoor[$i]['Latitude'] . "' ,
                lng: '" . $manycoor[$i]['Longitude'] . "'
            },";
            }
            ?>
