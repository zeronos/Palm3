...
if(isset($_POST['search'])){
    ...
    $sql = "SELECT `dim-user`.`Alias`, CONCAT(`dim-time`.`dd` , '/' , `dim-time`.`Month` , '/' ,`dim-time`.`Year2`) AS Date,`db-farm`.`Name` AS nfarm
    ,`db-subfarm`.`Name` AS nsf,`db-subfarm`.`AreaRai`,A.NumTree,MAX(`dim-time`.`Date`) AS date2,`log-activity`.`Note` AS note,`log-activity`.`PICs`
    ,`log-activity`.`ID`,`dim-user`.`FormalID`,`db-distrinct`.`AD2ID` AS DistrinctID,
    `db-province`.`AD1ID`,`db-subfarm`.`FSID`,`db-farm`.`FMID`,`dim-user`.`dbID` AS farmerID,`dim-time`.`Year2` AS year
    ...
    INNER JOIN (SELECT  (SUM(IF( lpt.`NumGrowth1` IS NULL , 0, lpt.`NumGrowth1`))+SUM(IF( lpt.`NumGrowth2` IS NULL , 0, lpt.`NumGrowth2`))-SUM(IF( lpt.`NumDead` IS NULL , 0, lpt.`NumDead`))) AS NumTree,lpt.`DIMsubFID` AS dsf FROM `log-planting` AS lpt
    WHERE  lpt.`isDelete` = 0
    GROUP BY  lpt.DIMsubFID) AS A ON A.dsf = `log-activity`.`DIMsubFID`
    INNER JOIN `db-subdistrinct` ON `db-subdistrinct`.`AD3ID` = `db-subfarm`.`AD3ID`
    INNER JOIN `db-distrinct`ON `db-distrinct`.`AD2ID` = `db-subdistrinct`.`AD2ID`
    ...
    WHERE `log-activity`.`isDelete`=0 AND `log-activity`.`DBactID`=1  AND `log-activity`.`LOGloginID`=-1 AND `dim-user`.`Type` = 'F'
    AND `dim-time`.`Year2` = '$year' $text
